function [result] =stability_clustering_plot(da,k)
...
% DD=pdist(da);
% YY=squareform(DD);
% Y_1= mdscale(YY, 2, 'criterion','strain');
% Y1=Y_1(:,1);
...
% [sim] = kernal_sim(da);
[sim] = pair_relation(da,k);
% dist_line=pdist(dataed);
% sim=-squareform(dist_line);
...
% [~,max_dis]=max(diff(Z(:,end)));
% detial_k=length(locat)-max_dis(end);
...
%             bin_sim_ij=step1(sim_ij);
%             if sum(sum(bin_sim_ij))==0
%                 ij=0;
%             else
%             ij=sum(sum(sim_ij.*bin_sim_ij))/(sum(sum(bin_sim_ij)));
%             end
...
% figure();
% for i=1:k
%     aa=newliudata((find(newliudata(:,end)==i)),1);
%     plot(Y1(aa,1),Y1(aa,2),'.');
%     hold all
% end
% set(gca,'XLim',[0 1]);
% set(gca,'YLim',[0 1]);
